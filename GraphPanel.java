...
public class GraphPanel extends JPanel
	...
	protected Node edgeStart = null;
	protected Node edgeEnd = null;
	...
	{
		moveNode(dx,dy,edge.getNodeFrom());
		moveNode(dx,dy,edge.getNodeTo());
	}
	...
	protected void createPopupMenu(MouseEvent event, Edge edge){
		...
		menuItem = new JMenuItem("Delete this edge");
		menuItem.addActionListener((a) -> {
	...
	protected void createPopupMenu(MouseEvent event, Node node) {
		...
		menuItem.addActionListener((a)->{
			if(fill)
			{
				edgeStart=nodeUnderCursor;
				fill=false;
				
			}
			else if((!fill))
			{
				if(nodeUnderCursor!=edgeStart)
				{
					edgeEnd=nodeUnderCursor;
					graph.addEdge(new Edge(edgeStart,edgeEnd));
					repaint();
				}
				edgeStart=null;
				edgeEnd=null;
		...
		menuItem.addActionListener((action) -> {
			if(nodeUnderCursor!=null){
				graph.removeNode(nodeUnderCursor);
				for(Edge edge:graph.getEdges()){
					if(edge.isMouseOver(nodeUnderCursor.getX(), nodeUnderCursor.getY()))
						graph.removeEdge(edge);
					repaint();
				}
			}
